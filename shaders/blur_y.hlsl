...
float4 main(float2 uv : TEXCOORD0, uniform float radius : register(c1)) : COLOR0
{
    float4 color = tex2D(texSampler, uv);
    color.rgb *= weights[0];

    for (int i = 1; i < 3; ++i) {
        color.rgb += tex2D(texSampler, float2(uv.x, uv.y - texelHeight * offsets[i] * radius)).rgb * weights[i];
        color.rgb += tex2D(texSampler, float2(uv.x, uv.y + texelHeight * offsets[i] * radius)).rgb * weights[i];
    }
